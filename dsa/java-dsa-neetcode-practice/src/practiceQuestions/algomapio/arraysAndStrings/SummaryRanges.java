 ...
 * You can also solve this with a two-pointer technique, where one pointer marks the start of a range, and another pointer moves forward to find where the range ends. Once you detect a break in continuity, record the range and reset the pointers.
 *
 ...
 * Space Complexity: O(1) extra space (excluding the output list), as we only use a few pointers and intermediate variables.
 *
