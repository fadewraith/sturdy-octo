...
class OllamaQueryGenerator:
    ...
    #         available_models = [model.get('name') for model in models['models'] if 'name' in model]
    #         if self.model_name not in available_models:
    #             st.warning(f"Model {self.model_name} not found. Available models: {available_models}")
    #             if available_models:
    ...
    def generate_sql_query(self, prompt: str, tables_info: Dict[str, Any], database_schema: str) -> str:
        ...
        try:
            system_prompt = f"""
You are a MySQL SQL expert. Generate accurate SQL queries based on natural language prompts.

Database Schema:
{database_schema}

Available Tables and their information:
{json.dumps(tables_info, indent=2)}

Rules:
1. Generate ONLY the SQL query, no explanations
2. Use proper MySQL syntax
3. Include table joins when referenced in the prompt
4. Use appropriate WHERE conditions based on the prompt
5. Limit results to reasonable numbers (add LIMIT if not specified)
6. Use backticks around table and column names if they contain special characters
7. Be precise with column names and table relationships
